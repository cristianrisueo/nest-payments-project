...
export const PaymentSchema = new Schema(
  {
    ...
    currencyCode: {
      type: String,
      required: true,
      enum: ['USD', 'EUR', 'GBP'], // Only these currencies are supported
      maxlength: 3,
    },
    ...
    paymentMethodType: {
      type: String,
      required: true,
      enum: ['CREDIT_CARD', 'DEBIT_CARD'], // Only these payment methods are supported
    },
    ...
    status: {
      type: String,
      required: true,
      enum: Object.values(PaymentStatus), // All valid payment statuses
      default: PaymentStatus.PENDING, // Default status is PENDING
      index: true,
    },
